...
export default function MobileNavbar({ links, isOpen, setIsOpen, isScrolled }) {
  ...
  return (
    <>
      {/* 1. Mobile Top Bar (Static on load) */}
      ...
      {/* 2. Sticky Toggle (Contained to 1400px Content Grid) */}
      <div className="fixed top-6 left-0 w-full pointer-events-none z-[60]">
        <div className="max-w-[1400px] mx-auto px-6 md:px-10 flex justify-end">
          <button
            ...
            className={`pointer-events-auto bg-[var(--color-background)]/90 border border-[var(--color-surface-2)] backdrop-blur-md rounded-full p-2 transition-all duration-700 ease-[cubic-bezier(0.23,1,0.32,1)] cursor-pointer shadow-2xl ${
              isScrolled && !isOpen
      ...
      >
        ...
        <div
          className={`absolute top-0 right-0 h-full w-[85%] md:max-w-[400px] bg-[var(--color-background)] border-l border-[var(--color-surface-2)] transition-transform duration-700 ease-[cubic-bezier(0.7,0,0.3,1)] flex flex-col p-8 md:p-12 ${
          ...
          <div className="flex flex-col gap-5 md:gap-7 mt-12">
            {links.map((link) => (
              <Link
                key={link.name}
                href={link.href}
                className="text-3xl md:text-4xl font-semibold text-[var(--color-text-main)] tracking-tight hover:text-[var(--color-accent)] transition-colors cursor-pointer"
                onClick={() => setIsOpen(false)}
              >
                {link.name}
              </Link>
            ...
            <div className="flex gap-6 pb-4">
              {[Instagram, Linkedin, Facebook].map((Icon, i) => (
                <Link
                  key={i}
                  href="#"
                  className="text-[var(--color-text-secondary)] hover:text-[var(--color-text-main)] transition-colors cursor-pointer"
                >
                  <Icon size={24} />
                </Link>
              ))}
